...
const Dashboard = () => {
    ...
    }, [activeTab, backtestWindow, forecastScenario]); // Added forecastScenario to dependencies
                                    ...
                                    <div className="glass-card p-8 rounded-[2.5rem]">
                                        ...
                                        <div className="mt-8 space-y-4">
                                            {[
                                                ...
                                                { label: 'LOW 5%', color: 'amber', cls: 'C' },
                                            ].map(item => (
                                                <div key={item.cls} className="flex justify-between items-center group cursor-help">
                                                    <div className="flex items-center space-x-3">
                                                        <div className={`w-2.5 h-2.5 rounded-full bg-${item.color}-500 shadow-lg shadow-${item.color}-500/20`} />
                                                        <span className="text-xs font-bold text-slate-400 group-hover:text-white transition-colors tracking-widest italic">CLASS {item.cls}</span>
                                                        <span className="text-[10px] text-slate-600 font-black">[{item.label}]</span>
                                                    </div>
                                                    <span className="text-sm font-black text-white">
                                                        {((dashboardData.abc_classification[item.cls] / (dashboardData.abc_classification.A + dashboardData.abc_classification.B + dashboardData.abc_classification.C)) * 100).toFixed(0)}%
                                                    </span>
                                                </div>
                                            ))}
                                        </div>
                                                ...
                                                {inventoryPage.items.length > 0 ? (
                                                    inventoryPage.items.map((item, idx) => (
                                                        ...
                                                        >
                                                            ...
                                                            <td className="p-6">
                                                                <button className="p-2 rounded-lg hover:bg-slate-700 transition-colors text-slate-500 hover:text-white">
                            ...
                            >
                                        ...
                                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                                            ...
                                            <div className="flex items-center space-x-2 px-4 py-2 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
                                                ...
                                                <span className="text-[10px] font-black text-emerald-500 tracking-widest">
                                                    {backtestLoading ? 'CALCULATING...' : 'REAL-TIME VALIDATED'}
                                                </span>
                                            </div>
                                        </div>
                                        <div className="h-[380px] w-full">
                                            ...
                                            <ResponsiveContainer width="100%" height="100%">
                                                <LineChart data={backtestData ? backtestData.scenarios[forecastScenario] : []}>
                                                    ...
                                                    <Line
                                                        type="monotone"
                                                        dataKey="demand"
                                                        stroke={forecastScenario === 'optimistic' ? '#10b981' : forecastScenario === 'conservative' ? '#f43f5e' : '#0ea5e9'}
                                                        strokeWidth={4}
                                                        dot={{ r: 4 }}
                                ...
                                <div className="glass-card p-10 rounded-[2.5rem] bg-gradient-to-br from-slate-900/30 to-primary-900/10">
                                    <div className="flex flex-col md:flex-row items-center gap-10">
                                        ...
                                        <div>
                                            <h3 className="text-2xl font-black text-white tracking-widest italic mb-2 text-center md:text-left uppercase">Neural Recommendation Engine</h3>
                                            <p className="text-slate-500 mt-4 leading-relaxed text-sm max-w-2xl text-center md:text-left">
